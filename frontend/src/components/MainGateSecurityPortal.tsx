...
import { Camera, Clock, LogOut, Shield, LayoutDashboard, Smartphone } from 'lucide-react';
...
type Tab = 'overview' | 'logs' | 'verification' | 'ip-camera';

export default function MainGateSecurityPortal() {
  const [activeTab, setActiveTab] = useState<Tab>('overview');
  const { user, logout } = useAuth();

  const tabs = [
    { id: 'overview' as Tab, name: 'Overview', icon: LayoutDashboard },
    { id: 'logs' as Tab, name: 'Main Logs', icon: Clock },
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'logs':
        return <MainLogs />;
      case 'verification':
        return <LiveVerification key="live-verification" cameraId={1} isActive={activeTab === 'verification'} webcamOnly={true} />;
      case 'ip-camera':
        return <LiveVerification key="ip-camera-verification" cameraId={1} isActive={activeTab === 'ip-camera'} ipCameraOnly={true} />;
      default:
        return <Overview />;
    }
  };
        ...
        <aside className="w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col">
          <div className="p-6 border-b border-gray-200 dark:border-gray-700">
            ...
            <div className="flex items-center space-x-2 mt-2">
              ...
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Main Security Portal</p>
            </div>
          </div>
